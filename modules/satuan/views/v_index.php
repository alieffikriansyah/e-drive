<?php $this->load->view('layout/header', ['title' => 'Master Satuan']); ?>

<!-- Load SweetAlert2 -->
<script src="<?= base_url('assets/js/sweetalert2.min.js') ?>"></script>

<div class="flex flex-col md:flex-row justify-between items-center mb-6 border-l-4 border-mapul-yellow pl-4">
    <h1 class="text-3xl font-extrabold tracking-wide text-mapul-green uppercase">Manajemen Satuan</h1>
    <button onclick="openModal()" class="btn-mapul mt-4 md:mt-0">
        <i class="fa fa-plus-circle text-mapul-yellow"></i> Tambah Satuan
    </button>
</div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-gray-300 transform transition-all scale-95 opacity-0 duration-300" id="formModalDialog">
        <div class="mapul-modal-header">
            <h3 class="text-xl font-bold text-mapul-green-md flex items-center gap-2" id="modalTitle">
                <i class="fa fa-balance-scale text-mapul-yellow"></i> Tambah Satuan
            </h3>
        ...
        <form id="dataForm" onsubmit="submitForm(event)" class="p-6 space-y-4">
            ...
            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Nama Satuan</label>
                <input type="text" id="nama_satuan" name="nama_satuan" required class="mapul-input" placeholder="Misal: Kilogram, Porsi">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Singkatan</label>
                <input type="text" id="singkatan" name="singkatan" required class="mapul-input" placeholder="Misal: Kg, Prs">
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(baseUrl + 'satuan/load_data')
            .then(res => res.json())
            .then(res => {
                if (res.status) {
                    let html = '';
                    if (res.data.length === 0) {
                        html = `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada data satuan</td></tr>`;
                    } else {
                        res.data.forEach(d => {
                            ...
                            html += `
                                <tr class="${trClass} transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 font-bold text-gray-800">${escapeHtml(d.nama_satuan)}</td>
                                    <td class="px-6 py-4 text-gray-600 font-medium">${escapeHtml(d.singkatan || '-')}</td>
    ...
    function openModal() {
        ...
        modalTitle.innerHTML = '<i class="fa fa-balance-scale text-mapul-yellow"></i> Tambah Satuan';
    ...
    function editData(id) {
        fetch(baseUrl + 'satuan/get_satuan/' + id)
            .then(res => res.json())
            .then(res => {
                if(res.status) {
                    const data = res.data;
                    openModal();
                    modalTitle.innerHTML = '<i class="fa fa-edit text-mapul-yellow"></i> Edit Satuan';
    ...
    function submitForm(e) {
        ...
        const url = id ? baseUrl + 'satuan/edit/' + id : baseUrl + 'satuan/create';
    ...
    function deleteData(id) {
        Swal.fire({
            title: 'Hapus Satuan?', text: "Data akan di-soft delete", icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
            confirmButtonText: 'Ya, hapus!'
        }).then((result) => {
            if (result.isConfirmed) {
                fetch(baseUrl + 'satuan/delete/' + id, { method: 'POST' })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status) {
    ...
    function restoreData(id) {
        fetch(baseUrl + 'satuan/restore/' + id, { method: 'POST' })
            .then(res => res.json())
            .then(res => {
                if (res.status) {
