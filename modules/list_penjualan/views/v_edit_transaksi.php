<?php $this->load->view('layout/header', ['title' => 'Edit Transaksi Penjualan']); ?>
...
<!-- Header Nav -->
<div class="mb-4">
    <a href="<?= base_url('list_penjualan') ?>" class="inline-block bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-4 rounded-lg transition-colors">
        <i class="fa fa-arrow-left mr-2"></i> Kembali ke List Penjualan
    </a>
</div>

<div class="flex flex-col md:flex-row h-[calc(100vh-190px)] gap-4 overflow-hidden">

    <!-- LEFT SIDE: MENU PRODUK -->
    <div class="w-full md:w-2/3 flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <!-- Header Menu -->
        <div class="bg-gray-50 border-b border-gray-200 p-4 flex justify-between items-center z-10">
            <h2 class="text-xl font-extrabold text-mapul-green flex items-center gap-2">
                <i class="fa fa-utensils text-mapul-yellow"></i> Menu Resto (Cabang: <?= htmlspecialchars($penjualan['nama_cabang']) ?>)
            </h2>
        ...
        <div class="bg-blue-50 text-blue-800 p-3 text-xs border-b border-blue-200 flex items-center gap-2">
            <i class="fa fa-info-circle"></i> Anda sedang mengedit Nota: <strong><?= htmlspecialchars($penjualan['no_nota']) ?></strong>
        </div>
    ...
    <div class="w-full md:w-1/3 flex flex-col h-full bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden relative">
        <div class="bg-blue-600 text-white p-4 flex justify-between items-center z-10 shadow-sm">
            <h2 class="text-lg font-bold flex items-center gap-2">
                <i class="fa fa-shopping-cart text-mapul-yellow"></i> Pesanan (Edit)
            </h2>
            <button onclick="clearCart()" class="text-white hover:text-red-300 transition-colors text-sm font-medium bg-blue-700 px-3 py-1 rounded">
        ...
        <div class="bg-white border-t border-gray-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-10">
            ...
            <div class="flex justify-between items-end mb-4">
                ...
                <span class="font-black text-3xl text-blue-600" id="cartTotal">Rp 0</span>
            </div>
            <button onclick="openPaymentModal()" id="btnBayarUtama"
                class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl shadow-lg transition-all transform hover:scale-[1.02] flex justify-center items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                disabled>
                <i class="fa fa-save text-mapul-yellow text-xl"></i>
                <span class="text-lg tracking-wide uppercase">Update Transaksi</span>
            </button>
        </div>
    </div>
</div>
...
<div id="paymentModal"
    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-gray-300 transform transition-all scale-95 opacity-0 duration-300"
        ...
        <div class="bg-blue-600 text-white p-5 rounded-t-2xl flex justify-between items-center shrink-0">
            <h3 class="text-xl font-bold flex items-center gap-2">
                <i class="fa fa-cash-register text-mapul-yellow"></i> Konfirmasi Edit
            </h3>
        ...
        <form id="paymentForm" onsubmit="processPayment(event)" class="flex flex-col max-h-[85vh]">
            <div class="p-6 overflow-y-auto flex-1">
                <div class="text-center mb-6 bg-gray-50 rounded-xl p-4 border border-gray-100">
                    <p class="text-sm font-bold text-gray-500 uppercase tracking-widest mb-1">Total Tagihan Baru</p>
                    <p class="text-4xl font-black text-blue-600" id="modalTotalTagihan">Rp 0</p>
                </div>
                
                <div class="mb-4">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Cabang Transaksi <span class="text-red-500">*</span></label>
                    <select id="pay_cabang" required class="w-full border-2 border-gray-300 rounded-lg p-2.5 focus:outline-none focus:border-blue-500 font-bold text-gray-700 bg-gray-50">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Tanggal Transaksi <span class="text-red-500">*</span></label>
                    <input type="datetime-local" id="pay_tanggal" required step="1"
                        class="w-full border-2 border-gray-300 rounded-lg p-2.5 focus:outline-none focus:border-blue-500 font-bold text-gray-700 bg-gray-50">
                </div>

                <div class="mb-4">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-2">Metode Pembayaran</label>
                    <div class="grid grid-cols-2 gap-2">
                        <label class="cursor-pointer">
                            <input type="radio" name="metode_bayar" id="metode_tunai" value="tunai" class="peer hidden" onchange="toggleUangPasBtn()">
                            <div class="border border-gray-200 rounded-lg p-2 text-center peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 transition-colors font-bold text-sm">
                                <i class="fa fa-money-bill-wave block mb-1 text-lg"></i> Tunai
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="metode_bayar" id="metode_qris" value="qris" class="peer hidden" onchange="toggleUangPasBtn()">
                            <div class="border border-gray-200 rounded-lg p-2 text-center peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 transition-colors font-bold text-sm">
                                <i class="fa fa-qrcode block mb-1 text-lg"></i> QRIS
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="metode_bayar" id="metode_transfer" value="transfer" class="peer hidden" onchange="toggleUangPasBtn()">
                            <div class="border border-gray-200 rounded-lg p-2 text-center peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 transition-colors font-bold text-sm">
                                <i class="fa fa-exchange-alt block mb-1 text-lg"></i> Transfer
                            </div>
                        </label>
                        <label class="cursor-pointer">
                            <input type="radio" name="metode_bayar" id="metode_lainnya" value="lainnya" class="peer hidden" onchange="toggleUangPasBtn()">
                            <div class="border border-gray-200 rounded-lg p-2 text-center peer-checked:bg-blue-600 peer-checked:text-white peer-checked:border-blue-600 transition-colors font-bold text-sm">
                                <i class="fa fa-wallet block mb-1 text-lg"></i> Lainnya
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div class="mb-5 relative">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Nominal Dibayar <span class="text-red-500">*</span></label>
                    <div class="relative">
                        <span class="absolute left-3 top-2.5 text-gray-500 font-bold">Rp</span>
                        <input type="text" id="pay_bayar_display" onkeyup="formatCurrencyInput(this); calculateKembalian()" required
                            class="w-full pl-10 pr-4 py-3 border-2 border-gray-300 rounded-lg font-bold text-xl text-gray-800 focus:outline-none focus:border-blue-500 transition-colors"
                            placeholder="0">
                ...
                <div class="mb-4">
                    <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider mb-1">Keterangan (Opsional)</label>
                    <textarea id="pay_keterangan" rows="2" class="w-full border-2 border-gray-300 rounded-lg p-2 focus:outline-none focus:border-blue-500"></textarea>
                </div>

                <div class="mb-4 p-4 rounded-lg bg-gray-100 border border-gray-200 flex justify-between items-center">
            ...
            <div class="p-5 bg-white border-t border-gray-200 rounded-b-2xl shrink-0">
                <div class="flex gap-3">
                    <button type="button" onclick="closePaymentModal()"
                        class="w-1/3 py-3 bg-gray-50 text-gray-700 font-bold rounded-xl border border-gray-300 hover:bg-gray-100 transition-colors shadow-sm">Batal</button>
                    <button type="submit" id="btnProsesBayar"
                        class="w-2/3 bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl shadow-md transition-all flex justify-center items-center gap-2">
                        <i class="fa fa-save text-mapul-yellow"></i> <span>Simpan Edit</span>
                    </button>
                </div>
            </div>
        </form>
...
<script>
    ...
    const currentId = <?= $penjualan['id'] ?>;
    let cart = []; 
    
    // Initial data injection from PHP
    const initialItems = <?= json_encode($items) ?>;
    
    // Inject initial items into cart
    initialItems.forEach(item => {
        cart.push({
            id: parseInt(item.id_harga_paten),
            nama: item.nama_menu,
            harga: parseFloat(item.harga_jual_paten),
            qty: parseFloat(item.jumlah_beli),
            diskon: parseFloat(item.diskon)
        });
    });

    // Populate initial payment data
    const initialPenjualan = <?= json_encode($penjualan) ?>;
    document.addEventListener('DOMContentLoaded', () => {
        renderCart();
        
        document.getElementById('pay_cabang').value = initialPenjualan.id_cabang;

        let dt = initialPenjualan.tanggal_transaksi;
        if(dt) dt = dt.replace(' ', 'T');
        document.getElementById('pay_tanggal').value = dt;
        
        const metode = initialPenjualan.metode_bayar || 'tunai';
        const radio = document.getElementById('metode_' + metode);
        if(radio) radio.checked = true;

        document.getElementById('pay_keterangan').value = initialPenjualan.keterangan || '';
        document.getElementById('pay_bayar').value = parseFloat(initialPenjualan.bayar);
        document.getElementById('pay_bayar_display').value = formatNumber(initialPenjualan.bayar);
    });
        ...
        cart.forEach(item => {
            ...
            html += `
                <div class="cart-item cart-item-anim bg-white border border-gray-200 rounded-xl p-3 shadow-sm relative">
                    ...
                    <div class="flex justify-between items-center mb-2">
                        <div class="text-blue-600 font-bold text-sm">Rp ${formatNumber(item.harga)}</div>
                        ...
                        <div class="flex items-center bg-gray-100 rounded-lg p-0.5 border border-gray-200">
                            ...
                            <button onclick="updateQty(${item.id}, 1)" class="w-7 h-7 flex items-center justify-center text-blue-600 hover:bg-white rounded hover:shadow-sm transition-all"><i class="fa fa-plus text-xs"></i></button>
                        </div>
                    </div>
    ...
    function openPaymentModal() {
        ...
        // Recalculate based on existing pay_bayar_display value if any, otherwise default to full total
        let currentBayar = document.getElementById('pay_bayar_display').value;
        if (!currentBayar) {
             setUangPas();
        } else {
             calculateKembalian();
    ...
    function calculateKembalian() {
        ...
        document.getElementById('pay_bayar').value = val;
    ...
    function processPayment(e) {
        ...
        const keterangan = document.getElementById('pay_keterangan').value;
        const cabang = document.getElementById('pay_cabang').value;

        // Prepare payload
        const payload = {
            id_cabang:    cabang,
            items: cart.map(item => ({
                id_harga_paten: item.id,
            ...
            tanggal_transaksi: tanggal.replace('T', ' ')
        };

        const btn = document.getElementById('btnProsesBayar');
        const originalHtml = btn.innerHTML;

        btn.innerHTML = '<i class="fa fa-spinner fa-spin text-mapul-yellow"></i> <span>Menyimpan...</span>';
        btn.disabled = true;

        fetch(baseUrl + 'list_penjualan/update_full/' + currentId, {
            method: 'POST',
            ...
            .then(res => {
                if (res.status) {
                    ...
                    Swal.fire({
                        ...
                        window.location.href = baseUrl + 'list_penjualan';
                    });
                } else {
