<?php $this->load->view('layout/header', ['title' => 'Master Kategori Operasional']); ?>

<!-- Load SweetAlert2 -->
<script src="<?= base_url('assets/js/sweetalert2.min.js') ?>"></script>

<div class="flex flex-col md:flex-row justify-between items-center mb-6 border-l-4 border-mapul-yellow pl-4">
    <h1 class="text-3xl font-extrabold tracking-wide text-mapul-green uppercase">Kategori Operasional</h1>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-gray-200 text-sm" id="tableBody">
                <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500"><i class="fa fa-spinner fa-spin mr-2"></i> Memuat data...</td></tr>
            </tbody>
        </table>
    </div>
</div>
...
<div id="formModal" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex justify-center items-center backdrop-blur-sm transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto border border-gray-300 transform transition-all scale-95 opacity-0 duration-300" id="formModalDialog">
        ...
        <form id="dataForm" onsubmit="submitForm(event)" class="p-6 space-y-4">
            ...
            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Nama Kategori <span class="text-red-500">*</span></label>
                <input type="text" id="nama_kategori" name="nama_kategori" required class="mapul-input" placeholder="Misal: Listrik, Gaji, dll">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Keterangan Tambahan</label>
                <textarea id="keterangan" name="keterangan" rows="3" class="mapul-input" placeholder="Opsional"></textarea>
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(baseUrl + 'kategori_operasional/load_data')
            .then(res => res.json())
            .then(res => {
                if (res.status) {
                    let html = '';
                    if (res.data.length === 0) {
                        html = `<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada data kategori operasional</td></tr>`;
                    } else {
                        res.data.forEach(d => {
                            const isDeleted = parseInt(d.status) === 8;
                            const trClass = isDeleted ? 'bg-red-50 hover:bg-red-100 line-through text-gray-400' : 'hover:bg-gray-50';
                            ...
                            html += `
                                <tr class="${trClass} transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 font-bold ${isDeleted ? '' : 'text-gray-800'}">${escapeHtml(d.nama_kategori)}</td>
                                    <td class="px-6 py-4 text-gray-600 font-medium">${escapeHtml(d.keterangan || '-')}</td>
    ...
    function openModal() {
        ...
        modalTitle.innerHTML = '<i class="fa fa-tags text-mapul-yellow"></i> Tambah Kategori Ops';
    ...
    function editData(id) {
        fetch(baseUrl + 'kategori_operasional/get_kat_ops/' + id)
            .then(res => res.json())
            .then(res => {
                if(res.status) {
                    const data = res.data;
                    openModal();
                    modalTitle.innerHTML = '<i class="fa fa-edit text-mapul-yellow"></i> Edit Kategori Ops';
    ...
    function submitForm(e) {
        ...
        const url = id ? baseUrl + 'kategori_operasional/edit/' + id : baseUrl + 'kategori_operasional/create';
    ...
    function deleteData(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(baseUrl + 'kategori_operasional/delete/' + id, { method: 'POST' })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status) {
    ...
    function restoreData(id) {
        fetch(baseUrl + 'kategori_operasional/restore/' + id, { method: 'POST' })
            .then(res => res.json())
            .then(res => {
                if (res.status) {
