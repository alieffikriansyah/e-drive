<?php $this->load->view('layout/header', ['title' => 'Master Cabang']); ?>

<!-- Load SweetAlert2 -->
<script src="<?= base_url('assets/js/sweetalert2.min.js') ?>"></script>

<div class="flex flex-col md:flex-row justify-between items-center mb-6 border-l-4 border-mapul-yellow pl-4">
    <h1 class="text-3xl font-extrabold tracking-wide text-mapul-green uppercase">Manajemen Cabang</h1>
    <button onclick="openModal()" class="btn-mapul mt-4 md:mt-0">
        <i class="fa fa-plus-circle text-mapul-yellow"></i> Tambah Cabang
    </button>
</div>
    ...
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-gray-300 transform transition-all scale-95 opacity-0 duration-300" id="formModalDialog">
        <div class="mapul-modal-header">
            <h3 class="text-xl font-bold text-mapul-green-md flex items-center gap-2" id="modalTitle">
                <i class="fa fa-store text-mapul-yellow"></i> Tambah Cabang
            </h3>
        ...
        <form id="dataForm" onsubmit="submitForm(event)" class="p-6 space-y-4">
            ...
            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Nama Cabang</label>
                <input type="text" id="nama_cabang" name="nama_cabang" required class="mapul-input" placeholder="Misal: Pusat Sudirman">
            </div>
            
            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Telepon</label>
                <input type="text" id="telepon" name="telepon" class="mapul-input" placeholder="Misal: 08123456789">
            </div>

            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Alamat</label>
                <textarea id="alamat" name="alamat" rows="3" class="mapul-input" placeholder="Alamat lengkap cabang"></textarea>
            </div>
            
            <div class="space-y-1">
                <label class="block text-xs font-bold text-gray-600 uppercase tracking-wider">Keterangan Tambahan</label>
                <textarea id="keterangan" name="keterangan" rows="2" class="mapul-input" placeholder="Catatan opsional"></textarea>
            </div>
...
<script>
    ...
    function loadData() {
        ...
        fetch(baseUrl + 'cabang/load_data')
            .then(res => res.json())
            .then(res => {
                if (res.status) {
                    let html = '';
                    if (res.data.length === 0) {
                        html = `<tr><td colspan="7" class="px-6 py-4 text-center text-gray-500">Tidak ada data cabang</td></tr>`;
                    } else {
                        res.data.forEach(d => {
                            ...
                            html += `
                                <tr class="${trClass} transition-colors duration-150">
                                    ...
                                    <td class="px-6 py-4 text-gray-600">${escapeHtml(d.alamat || '-')}</td>
                                    <td class="px-6 py-4 text-gray-600">${escapeHtml(d.telepon || '-')}</td>
                                    <td class="px-6 py-4 text-gray-500 italic text-xs">${escapeHtml(d.keterangan || '-')}</td>
    ...
    function openModal() {
        ...
        modalTitle.innerHTML = '<i class="fa fa-store text-mapul-yellow"></i> Tambah Cabang';
    ...
    function editData(id) {
        fetch(baseUrl + 'cabang/get_cabang/' + id)
            .then(res => res.json())
            .then(res => {
                if(res.status) {
                    const data = res.data;
                    openModal();
                    modalTitle.innerHTML = '<i class="fa fa-edit text-mapul-yellow"></i> Edit Cabang';
                    btnSubmitText.textContent = 'Simpan Perubahan';
                    
                    document.getElementById('id').value = data.id;
                    document.getElementById('nama_cabang').value = data.nama_cabang;
    ...
    function submitForm(e) {
        ...
        const url = id ? baseUrl + 'cabang/edit/' + id : baseUrl + 'cabang/create';
    ...
    function deleteData(id) {
        Swal.fire({
            title: 'Hapus Cabang?', text: "Data akan di-soft delete", icon: 'warning',
            showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
            confirmButtonText: 'Ya, hapus!'
        }).then((result) => {
            if (result.isConfirmed) {
                fetch(baseUrl + 'cabang/delete/' + id, { method: 'POST' })
                    .then(res => res.json())
                    .then(res => {
                        if (res.status) {
    ...
    function restoreData(id) {
        fetch(baseUrl + 'cabang/restore/' + id, { method: 'POST' })
            .then(res => res.json())
            .then(res => {
                if (res.status) {
