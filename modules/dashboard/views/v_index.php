    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="stat-card">
            <div class="flex justify-between items-start">
                ...
                <div
                    class="w-12 h-12 rounded-2xl bg-purple-50 flex items-center justify-center text-purple-600 shadow-sm border border-purple-100">
        ...
        <?php if ($is_admin): ?>
            <div class="stat-card">
                <div class="flex justify-between items-start">
                    ...
                    <div
                        class="w-12 h-12 rounded-2xl bg-orange-50 flex items-center justify-center text-orange-600 shadow-sm border border-orange-100">
            ...
            <div class="stat-card">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <h3 class="text-lg font-bold text-edrive-text leading-tight mt-1">
                            <?= htmlspecialchars(Session::get('role_name')) ?></h3>
                    </div>
                    <div
                        class="w-12 h-12 rounded-2xl bg-gray-50 flex items-center justify-center text-gray-600 shadow-sm border border-gray-200">
        ...
        <div class="<?= $is_admin ? 'lg:col-span-2' : 'lg:col-span-3' ?> space-y-6">
            ...
            <div class="glass-card overflow-hidden">
                <div class="p-5 border-b border-edrive-border flex items-center justify-between bg-white">
                    ...
                    <a href="<?= site_url('recent') ?>"
                        class="text-sm text-edrive-accent hover:underline font-medium">Lihat Semua</a>
                </div>
                    ...
                    <table class="table-light">
                        ...
                        <tbody>
                            <?php if (empty($recent_docs)): ?>
                                <tr>
                                    <td colspan="4" class="text-center py-8 text-edrive-muted">Belum ada dokumen yang
                                        diunggah</td>
                                </tr>
                            <?php else: ?>
                                    ...
                                    <tr>
                                        <td>
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-medium text-edrive-text line-clamp-1"
                                                        title="<?= htmlspecialchars($doc->name) ?>">
                                                        <?= htmlspecialchars($doc->name) ?>
                                                    </p>
                                                    <p class="text-xs text-edrive-muted">
                                                        <?= htmlspecialchars($doc->uploader_name) ?></p>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="badge badge-info bg-blue-50 text-blue-600 border border-blue-100">
                                                ...
                                                <?= htmlspecialchars($doc->drive_name) ?>
                                            </span>
                                        </td>
            ...
            <div class="lg:col-span-1">
                <div class="glass-card p-0 h-full flex flex-col">
                    <div class="p-5 border-b border-edrive-border flex items-center justify-between bg-white rounded-t-2xl">
                        ...
                        <a href="<?= site_url('activity_log') ?>"
                            class="text-sm text-edrive-accent hover:underline font-medium">Log</a>
                    </div>
                        ...
                        <?php else: ?>
                            <div class="relative border-l border-edrive-border ml-3 space-y-6 pb-4">
                                <?php foreach ($activities as $act): ?>
                                    ...
                                    if ($act->action == 'UPLOAD' || $act->action == 'CREATE') {
                                        $icon = 'fa-arrow-up text-emerald-500';
                                        $bg = 'bg-emerald-50';
                                        $border = 'border-emerald-200';
                                    } elseif ($act->action == 'DOWNLOAD') {
                                        ...
                                        $icon = 'fa-right-to-bracket text-purple-500';
                                        $bg = 'bg-purple-50';
                                    ...
                                    <div class="relative pl-6">
                                        <span
                                            class="absolute -left-[17px] top-1 w-8 h-8 rounded-full border shadow-sm flex items-center justify-center <?= $bg ?> <?= $border ?>">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (chartDataRaw.length > 0 && document.getElementById('storageChart')) {
        ...
        // Inverse arrays so the first item appears at the top
        categories.reverse();
        dataSeries.reverse();
        ...
        const option = {
            ...
            yAxis: {
                ...
                axisLabel: { color: '#64748B', fontFamily: 'Inter', fontSize: 11, width: 100, overflow: 'truncate' }
            },
            series: [
                {
                    name: 'Storage',
